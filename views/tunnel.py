...
from models import TunnelFeature, Project, ProjectUnit
...
def tunnel_list():
    ...
    # 查询数据库，结合分页
    query = TunnelFeature.query.join(ProjectUnit).join(Project).filter(and_(*filters))
...
def perform_calculation(projects):
    ...
    for project in projects:
            ...
            road_length = project.隧道长度 or 1  # 避免除以 0
            road_area = project.隧道面积 or 1  # 避免除以 0
...
#将将一个字典添加到 results 列表中，results 是一个列表，append() 方法会将一个新元素添加到该列表中。
            results.append({
                ...
                "单位工程": "隧道工程"+("/" + str(project.备注) if project.备注 else ""),
            })
