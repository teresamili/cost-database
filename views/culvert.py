...
from models import CulvertFeature, Project, ProjectUnit
from sqlalchemy import and_

culvert_blueprint = Blueprint('culvert', __name__)

@culvert_blueprint.route('/culverts', methods=['GET'])
def culvert_list():
    """
    ...
    # 查询数据库，结合分页
    query = CulvertFeature.query.join(ProjectUnit).join(Project).filter(and_(*filters))
...
def perform_calculation(projects):
    ...
    for project in projects:
            ...
            road_length = project.涵洞长度 or 1  # 避免除以 0
            road_area = project.涵洞面积 or 1  # 避免除以 0
