...
from models import WaterpipeFeature, Project, ProjectUnit,Unit
from sqlalchemy import and_

waterpipe_blueprint = Blueprint('waterpipe', __name__)

@waterpipe_blueprint.route('/waterpipes', methods=['GET'])
def waterpipe_list():
    """
    ...
    # 查询数据库，结合分页
    query =WaterpipeFeature.query.join(ProjectUnit).join(Project).filter(and_(*filters))
...
#将将一个字典添加到 results 列表中，results 是一个列表，append() 方法会将一个新元素添加到该列表中。
            results.append({
                ...
                "单位工程": "给水工程"+("/" + str(project.备注) if project.备注 else ""),
            })
