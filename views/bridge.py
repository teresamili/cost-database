...
from models import BridgeFeature, Project, ProjectUnit
from sqlalchemy import and_

bridge_blueprint = Blueprint('bridge', __name__)

@bridge_blueprint.route('/bridges', methods=['GET'])
def bridge_list():
    """
    ...
    # 查询数据库，结合分页
    query = BridgeFeature.query.join(ProjectUnit).join(Project).filter(and_(*filters))
...
def perform_calculation(projects):
    ...
    for project in projects:
            ...
            road_length = project.桥梁长度 or 1  # 避免除以 0
            road_area = project.桥梁面积 or 1  # 避免除以 0
...
#将将一个字典添加到 results 列表中，results 是一个列表，append() 方法会将一个新元素添加到该列表中。
            results.append({
                ...
                "单位工程": "桥梁工程"+("/" + str(project.备注) if project.备注 else ""),
            })
