...
from models import ElectricalFeature, Project, ProjectUnit,Unit
from sqlalchemy import and_

electrical_blueprint = Blueprint('electrical', __name__)

@electrical_blueprint.route('/electricals', methods=['GET'])
def electrical_list():
    """
    ...
    # 查询数据库，结合分页
    query = ElectricalFeature.query.join(ProjectUnit).join(Project).filter(and_(*filters))
...
def perform_calculation(projects):
    ...
    for project in projects:
            ...
            cost_index_electrical = project.工程造价/project.管线长度
...
#将将一个字典添加到 results 列表中，results 是一个列表，append() 方法会将一个新元素添加到该列表中。
            results.append({
                ...
                "管线指标": round(float(cost_index_electrical), 2),
                "单位工程": "电力工程"+("/" + str(project.备注) if project.备注 else ""),
            })
