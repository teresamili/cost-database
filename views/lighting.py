...
lighting_blueprint = Blueprint('lighting', __name__)

@lighting_blueprint.route('/lightings', methods=['GET'])
def lighting_list():
    """
    ...
    # 查询数据库，结合分页
    query = LightingFeature.query.join(ProjectUnit).join(Project).filter(and_(*filters))
