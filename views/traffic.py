...
from models import TrafficFeature, Project, ProjectUnit,Unit
from sqlalchemy import and_

traffic_blueprint = Blueprint('traffic', __name__)

@traffic_blueprint.route('/traffics', methods=['GET'])
def traffic_list():
    """
    ...
    # 查询数据库，结合分页
    query = TrafficFeature.query.join(ProjectUnit).join(Project).filter(and_(*filters))
...
#将将一个字典添加到 results 列表中，results 是一个列表，append() 方法会将一个新元素添加到该列表中。
            results.append({
                ...
                "交叉口指标": round(float(cost_index), 2),
                "单位工程": "交通工程"+("/" + str(project.备注) if project.备注 else ""),
            })
